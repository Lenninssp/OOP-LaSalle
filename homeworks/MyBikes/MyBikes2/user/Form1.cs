...
namespace MyBikes
{
    public partial class Form1 : Form
    {
        ...
        {
            //Get selected Bike
            Bike selectedBike = GetSelectedBike(); //We implement the method to get the bike

            //Get the newSpeed
            double newSpeed = (double)numericUpDownSpeed.Value;

            //Check if the newSpeed exceeds the maximum speed of each bike
            if (newSpeed > selectedBike.GetMaxSpeed())
            {
                MessageBox.Show($"The new speed ({newSpeed} km/h) exceeds the maximum speed of the bike ({selectedBike.GetMaxSpeed()} km/h).");
            }
